...
local sectorList = {
    ...
    -- Azkaban (ex. Tixa)
    { location = "G10-1", loadscreens = { "LS_Basement", "LS_Upstairs" }, },

    -- feeding zone
    { location = "G10-2", loadscreen = "LS_Cave", },


    -- Nasa (ex. Orta)
    { location = "A9-1", loadscreens = { "LS_Basement", "LS_Upstairs" }, },
   
    -- Gryfindor (ex. Meduna)
    { location = "A1-1", loadscreens = { "LS_Basement", "LS_Upstairs" }, },
    
    
    -- Duranth (ex. San Mona) mine
    { location = "K2-1", },
    { location = "K1-1", },
    { location = "K1-2", },
    { location = "K1-3", },   
    
    -- LodenBurg (ex. Drassen) mine
    { location = "J10-1", },
    { location = "J11-1", },
    { location = "J11-2", },
    { location = "J11-3", },
...
local function PopulateSectors(difficultyLevel, gameStyle)
    ...
    -- Azkaban (ex. Tixa)
    s["G10-1"].numTroops = ({ 8, 10, 13, 16 })[difficultyLevel]
                        + math.random(0, ({ 1, 2, 3, 4 })[difficultyLevel])
    s["G10-1"].numElites = 4 +   difficultyLevel + math.random(0, 1) 
 
    s["G10-2"].numCreatures = 6 + 2*difficultyLevel + math.random(0, 2)

    -- Nasa (ex. Orta)
    s["A9-1"].numTroops = 6 + 2*difficultyLevel + math.random(0, 2)
    s["A9-1"].numElites = 8 + difficultyLevel + math.random(0, 2)

    -- Gryfindor (ex. Meduna)
    s["A1-1"].numElites = ({ 15, 18, 21, 25 })[difficultyLevel]
                        + math.random(0, ({ 2, 3, 4, 5 })[difficultyLevel])
