...
local function CheckConditionsForTriggeringCreatureQuest( sSectorX, sSectorY, bSectorZ )
	...
	-- SEC_J10
	if ( SectorEnemyControlled ( CALCULATE_STRATEGIC_INDEX(10, SectorY.MAP_ROW_J) ) == false ) then
		ubValidMines = ubValidMines + 1
	end
	
	-- SEC_D11
	if ( SectorEnemyControlled ( CALCULATE_STRATEGIC_INDEX(11, SectorY.MAP_ROW_D) ) == false ) then
		ubValidMines = ubValidMines + 1
	end	
	
	-- SEC_F14
	if ( SectorEnemyControlled ( CALCULATE_STRATEGIC_INDEX(14, SectorY.MAP_ROW_F) ) == false ) then 
		ubValidMines = ubValidMines + 1
	end	
	
	-- SEC_H5
	if ( SectorEnemyControlled ( CALCULATE_STRATEGIC_INDEX(5, SectorY.MAP_ROW_H) ) == false ) then
		ubValidMines = ubValidMines + 1
	end	
