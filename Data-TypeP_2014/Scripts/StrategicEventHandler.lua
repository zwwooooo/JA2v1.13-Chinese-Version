...
function HandleEarlyMorningEvents()
	...
	if (o > 1) then
		...
		if  (not ( ( gWorldSectorX == 10) and ( ( gWorldSectorY == 11) or gWorldSectorY == 11 ) and ( gbWorldSectorZ == 0 ) ))  then

			SetCharacterSectorX(Profil.FATHER,10)
			SectorY = CheckCharacterSectorY(Profil.FATHER)
		
			-- Swap his location
			if ( SectorY == 11 ) then
				SetCharacterSectorY(Profil.FATHER,11)
			else
				SetCharacterSectorY(Profil.FATHER,11)
			end
		end
	...
	if ( CheckLastDateSpokenTot(Profil.TONY) > 0 and not ( gWorldSectorX == 2 and gWorldSectorY == 9 and gWorldSectorZ == 0 )) then
		...
		if ( p <= iniCHANCE_TONY_AVAILABLE ) then 
			...
			AddNPCtoSector (Profil.TONY,2,9,0)
		else
	...
	if ( CheckLastDateSpokenTot(Profil.DEVIN) == 0 ) then
		...
		if ( CheckNPCData1 (Profil.DEVIN) > 3 ) then
			...
			if ( not ( (gWorldSectorX == Devinx) and (gWorldSectorY == Deviny) and (gWorldSectorZ == 0) ) ) then
				...
				--N7
				AddNPCtoSector (Profil.DEVIN,7,14,0)
			end
		end
	end
	...
	if ( CheckNPCLife(Profil.HAMOUS) > 0 and CheckMercIsDead (Profil.HAMOUS) == false and CheckMercIsDead (Profil.PROF_ICECREAM) == false and (not ( (gWorldSectorX == HAMOUSx) and (gWorldSectorY == HAMOUSy) and (gWorldSectorZ == 0) )) ) then
		...
		-- L5
		AddNPCtoSector (Profil.HAMOUS,5,12,0) 
		AddNPCtoSector (Profil.PROF_ICECREAM,5,12,0) 
	end
	...
	if ( CheckQuest(Quests.QUEST_ESCORT_TOURISTS) == qStatus.QUESTDONE and JOHNx == 9 and JOHNy == 11 and JOHNz == 0 ) then
	...
	if ( CheckQuest(Quests.QUEST_ESCORT_TOURISTS) == qStatus.QUESTDONE and MARYx == 9 and MARYy == 11 and MARYz == 0  ) then	
	...
	if CheckMiscFlags2(Profil.CARMEN,Flags2.PROFILE_MISC_FLAG2_DONT_ADD_TO_SECTOR) == 1 then
		...
		-- Move Carmen to J9
		AddNPCtoSector (Profil.CARMEN,9,10,0) 
		...
		if ( CARMENx ~= gWorldSectorX or CARMENy ~= gWorldSectorY ) then			
			AddNPCtoSector (Profil.CARMEN,9,10,0) -- J9
